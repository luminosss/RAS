...
async function loadPremiumStatus(){
 ...
 isUserPremium = data?.premium === true;
}
...
async function loadSwipe(){
  ...
  const container = document.getElementById("loadSwipe");
  container.innerHTML = "";
  ...
  for(const u of data){
    ...
    const res = await fetch("/predict-match", {
      ...
      body: JSON.stringify({ me: currentUser, user: u })
    });
    ...
    card.innerHTML = `
      <img src="${u.photo_url || 'https://picsum.photos/300'}" width="100%">
...
async function likeUser(){
  ...
  await checkMatch(me.id, selectedUser.id);
}
...
async function loadProfiles(){
 ...
 const { data } = await supabaseClient
  ...
  if(!user.premium){
 card.innerHTML += "<div>💎 Premium requis pour discuter</div>";
}
 console.log(data);
}
...
function openAdmin(){
 ...
 if(data?.is_admin){
 document.body.insertAdjacentHTML("beforeend","👑 ADMIN");
  if(data?.is_admin){
